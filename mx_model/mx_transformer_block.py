...
class TransformerBlock(NamedTuple):
  attn_layer: mx_attention.Attention
  mlp_layer: mx_mlp.MLP
...
def init_block(prng_key, batch_size, sequence_length, d_model, d_ff):
  ...
  attn_layer = mx_attention.init_attention(key0, batch_size, sequence_length, d_model)
  mlp_layer = mx_mlp.init_mlp(key1, d_model, d_ff)
...
def dropout(seq, drop, prng_key):
  ...
  return jnp.multiply(jnp.asarray(mask, jnp.float32), seq) / (1.0 - drop)
